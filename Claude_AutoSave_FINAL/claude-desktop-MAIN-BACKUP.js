// ============================================
// CLAUDE AUTO-SAVE V10.0.3 - STABLE VERSION
// ============================================
// Features:
...
// - V10.0.1: Better UTF-8 handling & logging
// - V10.0.2: Fixed memory leak in server
// - V10.0.3: Fixed dangerous interval clearing
// ============================================

// Track our own intervals to clean up properly
const smartSaveIntervals = [];
const smartSaveTimeouts = [];

// Clear only OUR old intervals (not all intervals globally!)
if (window.smartSaveCleanup) {
    window.smartSaveCleanup();
}

// Define cleanup function for next time
window.smartSaveCleanup = () => {
    smartSaveIntervals.forEach(id => clearInterval(id));
    smartSaveTimeouts.forEach(id => clearTimeout(id));
    smartSaveIntervals.length = 0;
    smartSaveTimeouts.length = 0;
    console.log('🧹 Cleaned up Smart Save timers');
};

// Clear old broken localStorage entries (safe cleanup)
...
console.log('🧹 Starting fresh with V10.0.3 stable system');
...
(() => {
    ...
    const FINGERPRINT_LENGTH = 2500;  // Increased for better unique identification
    ...
    let chatFingerprints = {};  // Chat name -> FIRST 2500 chars (immutable)
    ...
    const getChatName = () => {
        ...
        if (document.title && document.title !== 'Claude' && document.title !== 'Claude.ai') {
            ...
            // IGNORE untitled chats - wait for Claude to name them
            if (title.toLowerCase() === 'untitled' || title === '') {
                return '';  // Return empty string for untitled
            }
            
            console.log(`🔍 Detected chat: "${title}"`);
            return title;
        }
        
        // Don't use fallback for new chats - wait for proper title
        return '';  // Return empty string
    };
    
    // Get conversation content (EXCLUDING our indicator!)
    ...
    const askForFolder = (chatName) => {
        return new Promise(async (resolve) => {
            ...
            // Get existing folders from server BEFORE creating modal
            let existingFolders = [];
            try {
            ...
            // Stop event propagation on modal to prevent interference
            ...
            // Handle folder selection with event.stopPropagation()
    ...
    const updateIndicator = async (status, color = null) => {
        const indicator = document.getElementById('autosave-indicator');
        if (indicator && !indicatorMinimized) {
            ...
            // Fetch real stats from server for current project
            if (currentFolder) {
                try {
                    const response = await fetch(`${SERVER_URL}/api/project/${encodeURIComponent(currentFolder)}/stats`);
                    if (response.ok) {
                        const projectStats = await response.json();
                        stats.words = projectStats.totalWords;
                        stats.messages = projectStats.chatCount; // Using chat count as approximation
                    }
                } catch (error) {
                    // Fall back to local calculation if server fetch fails
                    console.log('Could not fetch project stats, using local count');
                }
            }
    ...
    const startSession = async () => {
        try {
            ...
            if (response.ok) {
                sessionActive = true;
                console.log('✅ V9.3.0 Session started');
                await updateIndicator('Auto-Save Active', '#4ade80');
                return true;
            }
        } catch (error) {
            console.error('❌ Failed to start session:', error);
            await updateIndicator('Server Offline', '#ef4444');
            return false;
        }
    };
    ...
    const saveContent = async (content, chatName, folder, isFullCapture = false) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                // Don't calculate from current content - let updateIndicator fetch real stats
                console.log(`✅ ${isFullCapture ? 'Full capture' : 'Incremental'}: ${content.length} chars`);
                console.log(`   Chat: "${chatName}" → ${folder}/`);
                
                // Update indicator will fetch real stats from server
                await updateIndicator('Auto-Save Active', '#4ade80');
            ...
            await updateIndicator('Connection Error', '#f59e0b');
        }
    };
    
    // Monitor for changes
    const monitorContent = async () => {
        // Skip monitoring if modal is showing to prevent interference
        if (isShowingModal) {
            return;
        }
        ...
        if (chatName && chatName !== currentChatName) {
            // SKIP if this looks like a temporary name
            if (chatName.toLowerCase().includes('untitled')) {
                console.log(`⏳ Waiting for Claude to name this chat...`);
                return;
            }
    ...
    const initializeRealTimeSave = async () => {
        console.log('🚀 Initializing Claude Auto-Save V9.3.0 Ultimate...');
        ...
        // Start monitoring
        const monitorIntervalId = setInterval(monitorContent, UPDATE_INTERVAL);
        smartSaveIntervals.push(monitorIntervalId);  // Track our interval
    ...
    window.checkSave = () => {
        console.log('');
        console.log('╔═══════════════════════════════════════════════╗');
        console.log('║   CLAUDE AUTO-SAVE V9.0.6                    ║');
